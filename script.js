...
// -----------------------------DARK MODE-------------------------
let toggleStyle = document.getElementById("dark-mode-toggle");
...
// -----------------------POPULATE--------------------------------
let populateNBA = (data) => {
    let nbaRow = document.getElementById("player-rows")
    data.forEach(x => {
        let row =
            `<tr>
        <td> ${x.name} </td>
        ...
        nbaRow.insertAdjacentHTML('beforeend', row);
    });
}
populateNBA(data)
...
function filterByPlayer() {
    ...
    let filter = document.getElementById("team-filter").value;
    ...
    let column=getFilter(filter)
    doFilter(rows, column, word)
}
...
let searchNBA = document.getElementById("search");
searchNBA.addEventListener("keyup", filterByPlayer);

let selectFilter = document.getElementsByClassName("table-filters",)[0]
selectFilter.addEventListener("change", filterByBroadcast)